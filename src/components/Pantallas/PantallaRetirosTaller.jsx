...
const PantallaRetirosTaller = () => {
  ...
  useEffect(() => {
    const obtenerRetiros = async () => {
      const querySnapshot = await getDocs(collection(db, "retiros_taller"));
  ...
  const agregarRetiro = async (nuevoRetiro) => {
    const docRef = await addDoc(collection(db, "retiros_taller"), nuevoRetiro);
    setRetiros((prev) => [...prev, { id: docRef.id, ...nuevoRetiro }]);
  };

  const eliminarRetiro = async (id) => {
    await deleteDoc(doc(db, "retiros_taller", id)); 
