...
import { crearNota } from '../../hooks/useNotas';
...
const AltaNota = () => {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        const notaGuardado = await crearNota(form);
        if (notaGuardado) {
            alert("nota dada de alta correctamente");
            setForm({
              nota:"",
              texto:""
            });
        } else { 
            alert("Error al dar de alta la nota ");
        }
    };

    return (
        <Paper sx={{ p: 2, mt: 3, maxWidth: 450, margin: 'auto', backgroundColor: 'white' }}>
            <Typography textAlign="center" variant="h6" gutterBottom>Alta de Nota</Typography>
